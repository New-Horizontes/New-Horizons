		...
		if(REPRESENTATIVE_MISSION_HIGH)
			if(isvaurca(H))
				rep_objectives = pick("Obtain [rand(2,3)] sheets of solid phoron below market value, buying directly from the source.",
								"Compile and report information on Hegemony citizens who could potentially harbor anti-Hegemony sentiment.",
		...
		if(REPRESENTATIVE_MISSION_MEDIUM)
			if(isvaurca(H))
				rep_objectives = pick("Collect evidence of the [current_map.boss_name] being unfair or bigoted to Vaurca or Unathi Employees, to be used as leverage in future labor negotiations",
								...
								"Promote K'lax and Unathi culture to the crew of the [current_map.station_name]. Encourage tourism to Unathi space.")
			else
				rep_objectives = pick("Speak out against any violation of the Honor Code to or by Unathi on the [current_map.station_short]",
								"Proselytize your homeworlds religion to the crew.",
								"Discourage the Unathi crew of the [current_map.station_name] from the Aut'akh or Militant Sassurism heresies.")
		else
			if(isvaurca(H))
				...
				rep_objectives = pick("Ensure all Unathi crew are being respected in their beliefs and customs and traditions",
...
/datum/outfit/job/representative/consular/izweski
	name = "Unathi Hegemony Consular Officer"
