# The modularization handbook - New Horizons style, v0.1

## Failure to follow this guide will result in your PR being denied.

## Introduction
This handbook is stolen and modified from Rodent server (Skyrat).
...
One of the solutions for such, is to base our server on a solid codebase, that is primarily maintained by somebody else, in this case Aurora, and insert our content in a modular fashion, while following the general code (but not gameplay) direction of the upstream, mirroring any changes they do for parity.
...
```diff
- var/something = 1
+ var/something = 2 //NEW HORIZONS EDIT
```
...
**If it's an Aurora-specific tweak or bugfix, first course of action should be an attempt to discuss and PR it upstream, instead of needlessly modularizing it here.**

Otherwise, pick a new ID for your module. E.g. `XENOARCHEAOLOGY` or `SHUTTLE_TOGGLE` - We will use this in future documentation. It is essentially your module ID. It must be uniform throughout the entire module. All references MUST be exactly the same. This is to allow for easy searching.

And then you'll want to establish your core folder that you'll be working out of which is normally your module ID. E.g. `modular_newhorizons/modules/shuttle_toggle`
...
- ***/tg/ Example:*** You're adding a new lavaland mob.

  First of all you create your modular folder. E.g. `modular_newhorizons/modules/lavalandmob`
  ...
  ```byond
    /mob/lavaland/newmob
      icon = 'modular_newhorizons/modules/lavalandmob/icons/mob.dmi'
      icon_state = "dead_1"
      sound = 'modular_newhorizons/modules/lavalandmob/sounds/boom.ogg'
  ```
...
Example: `code/modules/mob/living/living.dm` -> `modular_newhorizons/master_files/code/modules/mob/living/living.dm`
...
In short, most of the modular code will be placed in the subfolders of your main module folder **`modular_newhorizons/modules/yourmodule/code/`**, with similar rules as with the assets. Do not mirror core code folder structures inside your modular folder.

For example, `modular_newhorizons/modules/xenoarcheaology/code` containing all the code, tools, items and machinery related to it.
...
- **Addition:**
  ...
  ```byond
  //NEW HORIZONS EDIT ADDITION BEGIN - SHUTTLE_TOGGLE - (Optional Reason/comment)
...
- **Removal:**
  ...
  ```byond
  //NEW HORIZONS EDIT REMOVAL BEGIN - SHUTTLE_TOGGLE - (Optional Reason/comment)
  /*
  for(var/obj/docking_port/stationary/S in stationary)
    if(S.id = id)
      return S
  */
  //NEW HORIZONS EDIT REMOVAL END
  ...
  ```byond
  //NEW HORIZONS EDIT REMOVAL BEGIN - SHUTTLE_TOGGLE - (Moved to modular_newhorizons/shuttle_toggle/randomverbs.dm)
  /*
...
- **Change:**
  ...
  ```byond
  //NEW HORIZONS EDIT CHANGE BEGIN - SHUTTLE_TOGGLE - (Optional Reason/comment)
  //if(SHUTTLE_STRANDED, SHUTTLE_ESCAPE) - NEW HORIZONS EDIT - ORIGINAL
...
That folder is **`code/__defines/~newhorizons_defines`**, in which you can add them to the existing files, or create those files as necessary.
...
- Good: /modular_newhorizons/modules/example_module/code/disease_mob.dm
- Bad: /modular_newhorizons/modules/example_module/code/modules/antagonists/disease/disease_mob.dm

**Icons**: Any .DMI files must go in here.

- Good: /modular_newhorizons/modules/example_module/icons/mining_righthand.dmi
- Bad: /modular_newhorizons/modules/example_module/icons/mob/inhands/equipment/mining_righthand.dmi
